...
parser = argparse.ArgumentParser(
    description="Lista de buses reportados en las últimas 24 horas"
)
...
# -------------------------------
# clave: "empresa:bus_nro", valor: lista de tópicos reportados y timestamp de la última vez que reportaron
buses_reportados = {}

# Tiempo límite (últimos 1 día)
TIEMPO_LIMITE = timedelta(days=1)

# -------------------------------
# Cliente origen (subscriber)
# -------------------------------
src_client = mqtt.Client(client_id="buses-reporte-diario")
...
def on_connect_src(client, userdata, flags, rc):
    ...
    # Suscribirse a todos los tópicos de todas las empresas
    client.subscribe("STMBuses/+/+/+/Vehiculo/EmpresaBus/3_0", qos=0)
    client.subscribe("STMBuses/+/+/+/DatosOperacionales/EstadoServicio/3_0", qos=0)
    client.subscribe("STMBuses/+/+/+/DatosOperacionales/EstadoOperativo/3_0", qos=0)
    client.subscribe("STMBuses/+/+/+/DatosOperacionales/Parada/3_0", qos=0)
    client.subscribe("STMBuses/+/+/+/DatosOperacionales/PuntoControl/3_0", qos=0)
    print(f"[SRC] Suscripto a todos los tópicos relevantes")
...
def on_message_src(client, userdata, msg):
    try:
        ...
        if len(topic_parts) >= 7 and topic_parts[0] == "STMBuses":
            ...
            try:
                ...
                if "MDTEntidad" in data:
                    ...
                    if now - timestamp <= TIEMPO_LIMITE:
                        ...
                        if clave not in buses_reportados:
                            buses_reportados[clave] = {"topicos": set(), "timestamp": timestamp.timestamp()}
                        buses_reportados[clave]["topicos"].add(f"{categoria}/{subcategoria}")
                        buses_reportados[clave]["timestamp"] = timestamp.timestamp()  # Actualizar con el último timestamp
                        print(f"*** REPORTE: Bus [Empresa:{empresa} Nro:{bus_nro} ID:{bus_id}] reportó en tópico {categoria}/{subcategoria}")
                    else:
                        print(f"*** AVISO: Mensaje descartado por ser antiguo (Bus Nro: {bus_nro})")
                else:
                    print(f"*** AVISO: mensaje sin campo MDTEntidad en {msg.topic}")
...
def volcar_resumen_a_excel():
    print("\n" + "="*50)
    print("RESUMEN DE BUSES QUE REPORTARON EN LAS ÚLTIMAS 24 HORAS:")
    if not buses_reportados:
        ...
        # Agregar encabezados
        ws.append(["Empresa", "Número de Bus", "ID de Bus", "Último Reporte", "Tópicos Reportados"])
        ...
        for clave, info in items_ordenados:
            ...
            topicos = ", ".join(info["topicos"])
            ws.append([empresa, bus_nro, bus_id, hora, topicos])
            print(f"  Empresa: {empresa}, Nro: {bus_nro}, ID: {bus_id} -> Último reporte: {hora}, Tópicos: {topicos}")
